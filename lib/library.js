	        ...
	        return function (action) {
	            ...
	            Promise.all(promiseList).then(function (responses) {
	                ...
	                responses.map(function (response, index) {
	                    ...
	                    if (test.meta.requestFailed && config.hooks.failed && config.hooks.failed(requestActions[index], response, store) === false || test.error && config.hooks.error && config.hooks.error(requestActions[index], response, store) === false) {
	                        interrupted = true;
	                    }
	                    if (test.error === true) {
	                        hadError = true;
